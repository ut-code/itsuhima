        ...
        <div className="container mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
          <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-8">
            <div className="mx-auto max-w-2xl text-center lg:mx-0 lg:text-left">
              <h1 className="mb-6 font-bold text-4xl text-base-content leading-[1.25] tracking-tight sm:text-5xl md:text-6xl">
                <span className="text-primary">「いつヒマ？」</span>で<br />
                日程調整しよう
              </h1>
              <p className="mx-auto max-w-lg text-base-content/70 text-lg lg:mx-0">
                とりあえずみんなの空いている時間を訊いてから、何を何時間やるか決めたい。そんな仲間うちでの調整に最適な、シンプルで直感的なツールです。
              </p>
              <NavLink
                to="/new"
                className="btn btn-primary btn-lg motion-safe:hover:-translate-y-0.5 mt-8 gap-2 px-8 py-6"
              >
                イベントを作成
...
function FeaturesSection() {
  const features = [
    {
      icon: <LuCircleCheck className="h-6 w-6 text-success" />,
      title: "候補日程は不要",
      description:
    ...
    {
      icon: <LuSmartphone className="h-6 w-6 text-info" />,
      title: "スマホでも簡単に入力可能",
      ...
      <div className="bg-base-100 py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto text-center">
            <h2 className="font-bold text-3xl text-base-content sm:text-4xl">イツヒマの特徴</h2>
          ...
          <div className="mt-16 grid gap-8 md:grid-cols-3">
            {features.map((feature) => (
              <div
                key={feature.title}
                className="flex flex-col rounded-xl border border-base-300 bg-base-100 p-8 shadow-sm"
              >
                <div className={`inline-flex h-14 w-14 items-center justify-center rounded-xl ${feature.color}`}>
                  {feature.icon}
                </div>
                <h3 className="mt-6 font-bold text-base-content text-xl">{feature.title}</h3>
                <p className="mt-3 text-base-content/70">{feature.description}</p>
              </div>
            ))}
          </div>
        </div>
